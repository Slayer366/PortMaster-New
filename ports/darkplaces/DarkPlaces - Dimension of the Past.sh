...
if [ ! -f "$GAMEDIR/dopa/pak0.pak" ]; then
    ./text_viewer.${DEVICE_ARCH} -f 25 -w -t "Missing gamedata" -m "Please place your pak0.pak into the ports/darkplaces/dopa directory.  Press 'Select' to exit this text viewer."
    exit 1
fi

if [ ! -f "$GAMEDIR/id1/pak1.pak" ]; then
    ./text_viewer.${DEVICE_ARCH} -f 25 -w -t "Missing gamedata" -m "Please place your registered pak1.pak into the ports/darkplaces/id1 directory (Dimension of the Past requires the full registered copy of Quake to run) and, optionally, the music tracks as track##.ogg in ports/darkplaces/id1/music.  Press 'Select' to exit this text viewer."
    exit 1
fi
