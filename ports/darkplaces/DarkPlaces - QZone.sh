...
#In the event that the user places their PAK1.PAK into the id1 directory and launches this Q-Zone! mod first.
...
if [ ! -f "$GAMEDIR/id1/pak1.pak" ]; then
    ./text_viewer.${DEVICE_ARCH} -f 25 -w -t "Missing gamedata" -m "Please place your registered pak1.pak into the ports/darkplaces/id1 directory (Q-Zone! requires the full registered copy of Quake to run) and, optionally, the music tracks as track##.ogg in ports/darkplaces/id1/music.  Press 'Select' to exit this text viewer."
    exit 1
fi
